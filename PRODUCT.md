...
Findafew helps people move from an activity they want to do to a small group and a practical plan. Success means the product makes discovery, group formation, coordination, and follow-through understandable and easier to review.

## Positioning

Findafew combines interests, personality, social context, and concrete activity planning so compatible people can form a group around something they intend to do, rather than browsing profiles as an end in itself.

## Operating Context

Members use Findafew across mobile and desktop for onboarding, browsing open plans, starting a group, messaging, checking upcoming activity, managing profiles, and adjusting privacy, safety, notification, and appearance preferences. The product is an installable PWA and must remain usable in low-light, bright-light, offline, compact, and assistive-technology contexts.
...
- Appearance preferences support account-backed mode, style, and palette values. Findafew uses only its current storage namespace and applies changes without reloads or hydration flashes.
