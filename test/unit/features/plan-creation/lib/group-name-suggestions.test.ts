...
import { buildGroupNameSuggestions } from "@/features/plan-creation/lib/group-identity/group-name-suggestions";
...
describe("buildGroupNameSuggestions", () => {
  ...
  it("uses exact Findafew category labels for fallback suggestions", () => {
    expect(
      buildGroupNameSuggestions({
  ...
  it("does not return the synthetic vocabulary used by the old pools", () => {
    ...
    expect(suggestions.join(" ")).not.toMatch(
      /\b(assembly|cohort|collective|flux|planCreation|squad|unit)\b/i,
    );
  });
});
