...
export function GifPickerPanel({ canSendGif, onSelect }: GifPickerPanelProps) {
  ...
  return (
    <div className="flex h-92 max-h-(--radix-popover-content-available-height) flex-col">
      <GiphySearchInput search={search} onSearchChange={setSearch} />
      <GiphyResultsGrid fetchGifs={fetchGifs} onSelect={onSelect} />
      <div className="border-border/50 border-t px-2 py-1.5 text-right font-bold text-muted-foreground text-xs">
        Powered by GIPHY
      </div>
    </div>
  );
