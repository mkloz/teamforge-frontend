...
import { ManualFriendInviteRow } from "@/features/forge/components/steps/step3-group/manual-friend-invite-row";
...
export function ParticipantInviteSlots({
  ...
  const friendsQuery = useInfiniteQuery({
    ...forgeFriendCandidatesQueryOptions(deferredSearch),
    enabled: isOpen,
  });
  const candidates =
  ...
  return (
    <div className="flex min-h-20 items-center gap-3 px-3 py-3">
      <Avatar
        ...
        name={name}
        className="size-11 shrink-0 ring-1 ring-forge-teal/35"
      />
