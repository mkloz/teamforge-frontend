import { cn } from "@/shared/lib/utils";
import { Button } from "@/shared/components/ui/button";
import { AnimatePresence, motion } from "framer-motion";
import { Plus } from "lucide-react";

import { ForgeHero } from "./components/forge-hero";
import { InlineForgeWizard } from "./components/inline-forge-wizard/index";
import { useForgeRouteState } from "./hooks/use-forge-route-state";

const STARTER_EXAMPLES = [
  {
    title: "Low-pressure",
    items: [
      "Coffee between classes",
      "A walk after work",
      "Board games Friday",
    ],
  },
  {
    title: "Active",
    items: ["Beginner climbing", "Five-a-side football", "Sunday cycle"],
  },
  {
    title: "Focused",
    items: ["Exam revision block", "Portfolio review", "Hack session"],
  },
] as const;

const FORGE_OUTCOMES = [
  ["Group", "A small set of people who fit the activity."],
  ["Invites", "Automatic search or manual picks, depending on how you start."],
  ["Chat", "A shared thread with the plan already attached."],
] as const;

export function ForgePage() {
  const { isOpen, openWizard, closeWizard } = useForgeRouteState();

  return (
    <div
      className={cn(
        "mx-auto flex h-full flex-col md:pb-12",
        isOpen
          ? "w-full max-w-none gap-0 px-0"
          : "w-full max-w-5xl gap-8 px-4 md:px-8",
      )}
    >
      <AnimatePresence mode="wait">
        {!isOpen ? (
          <motion.div
            key="forge-introduction"
            initial={{ opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -8 }}
            transition={{ duration: 0.25, ease: "easeOut" }}
            className="flex flex-col gap-8 py-6 md:py-10"
          >
            <ForgeHero onForgeClick={openWizard} />

            <section
              aria-labelledby="starter-examples-title"
              className="grid gap-5 md:grid-cols-[13rem_minmax(0,1fr)]"
            >
              <div>
                <p className="text-sm font-black uppercase text-muted-foreground">
                  Good starts
                </p>
                <h2
                  id="starter-examples-title"
                  className="mt-2 text-2xl font-black leading-tight text-balance text-foreground"
                >
                  Start with the kind of plan someone can answer quickly.
                </h2>
              </div>

              <div className="grid gap-3 sm:grid-cols-3">
                {STARTER_EXAMPLES.map(({ title, items }) => (
                  <article
                    key={title}
                    className="rounded-2xl border border-border bg-card p-4"
                  >
                    <h3 className="text-sm font-black text-foreground">
                      {title}
                    </h3>
                    <ul className="mt-3 grid gap-2">
                      {items.map((item) => (
                        <li
                          key={item}
                          className="border-t border-border pt-2 text-sm font-medium leading-relaxed text-muted-foreground first:border-t-0 first:pt-0"
                        >
                          {item}
                        </li>
                      ))}
                    </ul>
                  </article>
                ))}
              </div>
            </section>

            <section
              aria-labelledby="forge-next-title"
              className="rounded-2xl border border-border bg-canvas p-5 md:p-6"
            >
              <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
                <div>
                  <p className="text-sm font-black uppercase text-muted-foreground">
                    After you start
                  </p>
                  <h2
                    id="forge-next-title"
                    className="mt-2 max-w-2xl text-2xl font-black leading-tight text-balance text-foreground"
                  >
                    Forge turns the rough plan into a group space.
                  </h2>

                  <div className="mt-5 grid gap-4 sm:grid-cols-3">
                    {FORGE_OUTCOMES.map(([title, body]) => (
                      <div
                        key={title}
                        className="border-t border-border pt-4 first:border-t-0 first:pt-0 sm:border-l sm:border-t-0 sm:pl-4 sm:pt-0 sm:first:border-l-0 sm:first:pl-0"
                      >
                        <p className="text-sm font-black text-foreground">
                          {title}
                        </p>
                        <p className="mt-1 text-sm font-medium leading-relaxed text-muted-foreground">
                          {body}
                        </p>
                      </div>
                    ))}
                  </div>
                </div>

                <Button
                  onClick={() => openWizard()}
                  variant="primary"
                  size="lg"
                  className="w-full lg:w-auto"
                  aria-label="Forge my group"
                >
                  <Plus size={18} />
                  Forge my group
                </Button>
              </div>
            </section>
          </motion.div>
        ) : (
          <InlineForgeWizard key="forge-wizard" onCancel={closeWizard} />
        )}
      </AnimatePresence>
    </div>
  );
}
