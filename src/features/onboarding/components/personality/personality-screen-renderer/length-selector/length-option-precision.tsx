...
export function LengthOptionPrecision({
  ...
  return (
    <div className="flex flex-col justify-center sm:items-end sm:text-right">
      <p className="mb-2 font-sans text-xs font-medium leading-relaxed text-muted-foreground text-pretty sm:mb-1.5">
        {viewModel.config.sublabel}
      </p>

      <div className="flex w-full items-center gap-2 opacity-80 sm:justify-end">
        <span className="shrink-0 font-sans text-xs font-black uppercase tracking-wide text-muted-foreground">
          {getPrecisionLabel(isAdjust, viewModel)}
        </span>

        {showProgress ? (
          <div className="h-1.5 w-24 shrink-0 overflow-hidden rounded-full bg-slate-100 dark:bg-white/10 sm:w-28">
            <motion.div
              initial={{ width: 0 }}
