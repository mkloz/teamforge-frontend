...
export function CalculatingScreen({ vector, onDone }: CalculatingScreenProps) {
  ...
  useEffect(() => {
    ...
    const targetWidths = DIMS.map((dim) => toDisplayPercent(vector, dim));
    ...
    async function sequence() {
      ...
      try {
        ...
        if (mounted) finishCalculation();
      } catch (err) {
        console.error("Animation sequence failed:", err);
        if (mounted) finishCalculation(); // Fail-safe: transition anyway
      }
    }
    ...
    <div className="flex-1 flex flex-col items-center justify-center w-full px-4 sm:px-6 pb-12">
      ...
      <div className="flex flex-col gap-6 w-full max-w-sm">
        {DIMS.map((dim, i) => {
          ...
          const targetValue = toDisplayPercent(vector, dim);
