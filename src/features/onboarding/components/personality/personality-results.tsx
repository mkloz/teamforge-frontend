    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-sans text-micro text-slate-muted/70">
            {labels.lowLabel} → {labels.highLabel}
          </span>
        </div>
        <span className="font-sans text-micro font-semibold text-forge-teal">
          {levelLabel}
        </span>
      </div>
      <div className="relative w-full rounded-full overflow-hidden h-1.5 bg-slate-muted/10">
        <motion.div
          initial={{ width: "0%" }}
          animate={{ width: `${pct}%` }}
          transition={{ duration: 0.8, ease: "easeOut", delay }}
          className="absolute top-0 left-0 bottom-0 h-full rounded-full bg-linear-to-r from-forge-teal to-forge-teal/60"
        />
      </div>
    </div>
  );
...
export function PersonalityResults({
  ...
  return (
    <div className="flex flex-col max-w-lg mx-auto w-full gap-0">
      {/* Overline */}
      <p className="font-sans text-micro font-semibold uppercase tracking-[0.18em] mb-5 text-center text-forge-teal">
        ...
        {letters.map((letter, i) => {
          ...
          return (
            <motion.div
              ...
              className="flex flex-col items-center justify-center rounded-2xl w-spacing-17 h-spacing-20 bg-white border-thin border-forge-teal/20 shadow-sm gap-0"
            >
              {/* Letter */}
              <span className="font-sans font-extrabold leading-none text-4xl text-forge-teal">
                {letter}
              </span>
              {/* Teal underline accent */}
              <div className="w-5 h-0.5 bg-forge-teal rounded-full opacity-40 mt-0.75 mb-1" />
              {/* Axis label */}
              <span className="font-sans font-medium leading-none text-nano text-slate-muted/70 tracking-[0.04em]">
                {axisLabel}
              </span>
            </motion.div>
          );
      ...
      <p className="font-sans text-sm text-center mb-6 text-pretty text-slate-muted">
        {result.info.tagline}
      </p>

      {/* Description card */}
      <Card className="rounded-2xl p-5 mb-5 shadow-[0_1px_4px_rgba(0,0,0,0.04)] border-thin border-slate-muted/10 bg-white">
        <p className="font-sans text-sm leading-relaxed mb-3 text-pretty text-ink">
          {result.info.about}
        </p>
      ...
      {/* OCEAN bars */}
      <Card className="rounded-2xl p-5 mb-5 flex flex-col gap-4 shadow-[0_1px_4px_rgba(0,0,0,0.04)] border-thin border-slate-muted/10 bg-white">
        <p className="font-sans text-micro font-semibold uppercase tracking-widest text-slate-muted">
          Your Five Traits
        </p>
        {DIMS.map((dim, i) => (
          <SpectrumBar
      ...
      {/* Compatibility note */}
      <div className="flex items-start gap-2.5 rounded-xl p-3.5 mb-7 bg-slate-muted/5">
        <Info
          size={14}
          strokeWidth={2}
          className="text-slate-muted shrink-0 mt-px"
        />
        <p className="font-sans text-xs leading-relaxed text-slate-muted">
      ...
      <div className="flex flex-col gap-3">
        ...
        <Button
          ...
          size="lg"
          onClick={onRetake}
          className="w-full flex items-center justify-center gap-2 font-sans text-sm font-semibold rounded-xl transition-colors duration-200 border-primary/30 text-primary hover:bg-primary/5 bg-transparent h-12"
        >
        ...
        <Button
          ...
          size="lg"
          asChild
          className="w-full flex items-center justify-center gap-2 font-sans text-sm font-semibold rounded-xl text-slate-muted hover:text-ink hover:bg-slate-muted/10 transition-colors duration-200 h-12"
        >
          <a
      ...
      <p className="font-sans text-micro text-center mt-3 text-slate-muted/55">
