    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <button
          type="button"
          className={cn(
            "inline-flex min-h-11 min-w-11 max-w-full cursor-help items-center rounded-full border px-2.5 text-left font-bold text-xs leading-snug focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-forge-teal/35 sm:px-3 [@media(pointer:fine)]:min-h-7 [@media(pointer:fine)]:min-w-0 sm:[@media(pointer:fine)]:min-h-8",
            isSupporting
              ? "border-border/80 bg-canvas text-ink/70"
              : "border-forge-teal/20 bg-forge-teal/10 text-forge-teal dark:text-slate-muted",
          )}
        >
          {interest.name}
        </button>
