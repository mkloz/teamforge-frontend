...
export function ProfileAvatar({ src, name }: ProfileAvatarProps) {
  return (
    <div className="shrink-0 relative group">
      ...
      <div className="absolute -inset-1.5 border-2 border-forge-teal/30 rounded-full transition-all duration-700 opacity-0 group-hover:opacity-100 group-hover:rotate-180 group-hover:scale-105" />
      <img
        src={src}
        alt={name}
        className="relative w-38 h-38 sm:w-38 sm:h-38 rounded-full object-cover border-[3px] border-canvas bg-muted shadow-lg z-10 transition-transform duration-300 group-hover:scale-[1.02]"
      />
