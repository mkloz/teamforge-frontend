...
import {
  type CurrentGroupProposalResponse,
  groupProposalQueries,
} from "@/features/group-proposals/public/proposal-review";
import { CurrentGroupProposalStatus } from "@/features/home/components/current-group-proposal-status";
import { HomeSectionHeading } from "@/features/home/components/home-section-heading";
import {
  type AutomaticGroupFormationRequest,
  clearAutomaticGroupFormationRequestWizardDraft,
  saveAutomaticGroupFormationRequestAsWizardDraft,
  useAutomaticGroupFormationRequest,
} from "@/features/plan-creation/public/automatic-group-formation-request";
...
type CurrentGroupProposalQuery = UseQueryResult<CurrentGroupProposalResponse>;

export function AutomaticGroupFormationRequestStatus() {
  const state = useAutomaticGroupFormationRequest();
  const proposalQuery = useQuery(groupProposalQueries.current());

  if (state.isLoading) {
    return (
      <section className="grid gap-3" aria-label="Group request" role="status">
        ...
        <span className="sr-only">Loading your group request</span>
      </section>
    );
  }
  ...
  if (state.isStateError && !state.request) {
    return (
      <div className="grid gap-8">
        ...
        <section
          id="plan-creation-request-heading"
          className="grid gap-3"
          aria-label="Group request"
          role="alert"
        >
          <p className="text-muted-foreground text-sm">
            Your current group request could not be refreshed.
          </p>
          <Button
            variant="outline"
            size="sm"
...
function StandaloneProposalStatus({
  ...
  proposalQuery: CurrentGroupProposalQuery;
...
function RequestStatusCard({
  ...
  proposalQuery: CurrentGroupProposalQuery;
  request: AutomaticGroupFormationRequest;
  state: ReturnType<typeof useAutomaticGroupFormationRequest>;
}) {
  ...
  return (
    <section
      ...
      aria-labelledby="plan-creation-request-heading"
    >
      <HomeSectionHeading
        id="plan-creation-request-heading"
    ...
    return (
      <Button asChild className="w-fit">
        <Link
          {...buildPlanCreationLaunchNavigation()}
          onClick={() =>
            clearAutomaticGroupFormationRequestWizardDraft(request.id)
          }
        >
          Start a new request
        </Link>
...
function ProposedRequestAction({
  ...
  return (
    <Button asChild className="w-fit">
      <Link {...buildGroupProposalNavigation(proposal.id)}>
        Review group
        <ArrowRight className="size-4" aria-hidden="true" />
      </Link>
    </Button>
...
function CancelRequestAction({
  ...
  return (
    <ActionDialog
      ...
      title="Cancel this group request?"
      tone="danger"
      trigger={
...
function getRequestView(request: AutomaticGroupFormationRequest) {
  if (request.lifecycle === "DRAFT") {
    return {
  ...
  if (request.lifecycle === "SEARCHING") {
    return {
      label: "Searching",
      description: "Findafew checks automatically; no group has formed yet.",
    };
  }
  if (request.lifecycle === "PAUSED") {
    return {
...
function getPauseDescription(
  reason: AutomaticGroupFormationRequest["pauseReason"],
) {
  ...
  if (reason === "AUTOMATIC_GROUP_CAPACITY") {
    return "This search is paused while your current Findafew group limit is in use.";
  }
  ...
  return (
    <Button asChild>
      <Link
        ...
        onClick={() => saveAutomaticGroupFormationRequestAsWizardDraft(request)}
      >
        {label}
      </Link>
    </Button>
