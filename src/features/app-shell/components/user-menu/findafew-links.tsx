...
const FINDAFEW_LINKS = [
  { to: "/", label: "About Findafew" },
  { to: "/download", label: "Download" },
  { to: "/privacy", label: "Privacy policy" },
  { to: "/terms", label: "Terms of use" },
] as const;

export function FindafewLinks() {
  const { isStandalone } = usePwaDisplayMode();
  const links = isStandalone
    ? FINDAFEW_LINKS
    : FINDAFEW_LINKS.filter((link) => link.to !== "/download");

  return (
    <section
      aria-labelledby="findafew-links-title"
      className="border-border/50 border-t px-4 py-3"
    >
      <p
        id="findafew-links-title"
        className="px-3 pb-1.5 font-semibold text-muted-foreground text-xs"
      >
        Findafew
      </p>

      {!isStandalone ? <InstallFindafewLink /> : null}

      <nav
        aria-label="Findafew information"
  ...
  return (
    <SheetClose asChild>
      <Link
        ...
        aria-label="Install Findafew on your phone or computer"
      >
        <IconTile icon={Download} tone="neutral" size="md" />

        <span className="min-w-0 flex-1">
          <span className="block font-semibold text-sm">Install Findafew</span>
