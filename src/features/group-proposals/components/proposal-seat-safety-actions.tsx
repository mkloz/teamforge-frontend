...
import { clearGroupProposalSensitiveCaches } from "@/features/group-proposals/api/group-proposal-cache";
...
export function ProposalSeatSafetyActions({
  ...
  const blockMutation = useMutation({
    mutationKey: ["group-proposals", proposalId, "block-seat", seat.seatId],
    mutationFn: () => blockUser(seat.userId),
    meta: { errorToast: false },
    onSuccess: async () => {
      setBlockError(null);
      setIsBlocked(true);
      await clearGroupProposalSensitiveCaches(queryClient);
  ...
  async function blockProposalMember() {
    ...
    if (
      guardOfflineAction({
        ...
        id: `group-proposal-${proposalId}-block-offline`,
      })
    ) {
      ...
      <div className="flex flex-wrap gap-2 sm:justify-end">
        ...
        <ActionDialog
          ...
          details={[
            ...
            "Findafew will not tell them who blocked whom.",
