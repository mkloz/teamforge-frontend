...
function ProposalStatusState({
  ...
  return (
    <section
      aria-labelledby="group-proposal-state-heading"
    ...
    >
      ...
      <h1
        id="group-proposal-state-heading"
...
export function GroupProposalEmptyState({ action }: { action?: ReactNode }) {
  return (
    <ProposalStatusState
      icon={Inbox}
...
export function GroupProposalExpiredState({ action }: { action?: ReactNode }) {
  return (
    <ProposalStatusState
      icon={Clock3}
...
export function GroupProposalUnavailableState({
  action,
}: {
  action?: ReactNode;
}) {
...
export function GroupProposalCompleteState({ action }: { action?: ReactNode }) {
  return (
    <ProposalStatusState
      icon={UsersRound}
...
export function GroupProposalRecoveryWaitingState({
  action,
  holdUntil,
  refreshFailed = false,
}: {
  ...
  return (
    <ProposalStatusState
      ...
      description={`Findafew is holding your accepted place until ${formatHoldUntil(holdUntil)} while this proposal is resolved. A smaller group will not be created. If the roster changes, you will review it again before the group forms.`}
      statusMessage={
        refreshFailed
...
export function GroupProposalResponseSavedState({
  action,
  decision,
}: {
  action?: ReactNode;
...
export function GroupProposalErrorState({ onRetry }: { onRetry?: () => void }) {
  return (
    <ProposalStatusState
      icon={CircleOff}
...
export function GroupProposalLoadingState() {
  return (
    <div
