...
export function SuccessHero({
  ...
  return (
    <section className="overflow-hidden rounded-2xl bg-card ring-1 ring-brand-teal/20 ring-inset">
      <div className="sm:main-action-grid grid gap-5 px-4 py-5 sm:items-end sm:px-5">
        <div className="min-w-0">
          ...
          <p className="mt-1 max-w-xl text-muted-foreground text-sm leading-relaxed">
            This group includes {matchedCount}{" "}
        ...
        <div className="flex items-baseline gap-2 sm:block sm:text-right">
          ...
          <p className="mt-1 font-semibold text-muted-foreground text-xs">
            {averageScore === null ? "Group status" : "Average group fit"}
          </p>
        </div>
      </div>
        ...
        <SummaryFact
          ...
          detail={
            openSeatCount > 0
              ...
              : inviteeCount > 0
                ? `${matchedCount} suggested + ${inviteeCount} invited + you`
                : `${matchedCount} suggested + you`
          }
        />
        <SummaryFact
          icon={CalendarDays}
...
function getAverageScore(participants: FormationCandidate[]) {
