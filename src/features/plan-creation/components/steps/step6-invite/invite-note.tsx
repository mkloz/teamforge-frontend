...
export function InviteNote({
  ...
  const note = getInviteNoteViewState({ groupFormationMode, inviteeCount });
...
function getInviteNoteViewState({
  ...
  return {
    ...
    steps: getHandoffSteps(hasInvites, groupFormationMode),
  };
}
...
function getHandoffSteps(
  hasInvites: boolean,
  groupFormationMode: InviteNoteProps["groupFormationMode"],
): HandoffStep[] {
  return [
    {
      ...
      text: hasInvites
        ...
        : groupFormationMode === "AUTO"
          ? "The people Findafew found already have access."
