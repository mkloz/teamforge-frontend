  ...
  return (
    <div
      className={cn(
        ...
        !hasCover &&
          "bg-linear-to-br from-brand-teal/18 via-card to-brand-amber/12",
      )}
    >
      {hasCover ? (
        <PlanCover
        ...
        <div className="absolute inset-0 opacity-60" aria-hidden="true">
          <div className="absolute top-6 right-10 size-28 rounded-full bg-brand-teal/16 blur-3xl" />
          <div className="absolute bottom-2 left-12 size-24 rounded-full bg-accent-soft blur-3xl" />
        </div>
      )}
...
function getGroupSummaryViewState({
  ...
  return {
    ...
    statusLabel: getStatusLabel(groupFormationMode),
...
function getStatusLabel(
  groupFormationMode: GroupSummaryCardProps["groupFormationMode"],
) {
  return groupFormationMode === "MANUAL" ? "Ready to invite" : "Group ready";
}
