...
function getMissingPlanDetails(fw: PlanBuilderState) {
  ...
  if (!fw.canAdvanceStep2 && details.length === 0) {
    details.push({
      id: "plan-validation",
      label: fw.planCreationValidationMessage ?? "Review the plan details",
...
function getLocationDetail(fw: PlanBuilderState): MissingPlanDetail | null {
...
function getScheduleDetail(fw: PlanBuilderState): MissingPlanDetail | null {
...
function isPlaceReady(fw: PlanBuilderState) {
...
function isTimeReady(fw: PlanBuilderState) {
  if (fw.planScheduleMode === "TO_BE_DECIDED") {
    return !fw.planDate && !fw.planTime;
  }
