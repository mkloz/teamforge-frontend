...
import { PlanBuilderProgressBar } from "@/features/plan-creation/components/plan-creation-progress-bar";
import { ActionDialog } from "@/shared/components/ui/action-dialog";
import { Button } from "@/shared/components/ui/button";
import { IconTile } from "@/shared/components/ui/icon-tile";

import { getPlanCreationStepMetadata } from "./plan-creation-step-metadata";
import type { PlanBuilderChildProps } from "./types";

interface InlinePlanHeaderProps extends PlanBuilderChildProps {
  hasProgress: boolean;
  headingId: string;
...
export function InlinePlanHeader({
  fw,
  hasProgress,
  headingId,
  headingRef,
  ...
  showCancelDialog,
}: InlinePlanHeaderProps) {
  const currentMetadata = getPlanCreationStepMetadata(fw);
    ...
    <div className="sticky top-0 z-30 -mx-6 mb-2 border-border/40 border-b bg-transparent px-6 pb-3 shadow-black/5 shadow-sm backdrop-blur-xl md:-mx-12 md:px-12">
      <div className="mb-2 flex items-center justify-between pt-4 md:mb-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          {fw.isPrePlanCreation && (
            <ActionDialog
              cancelLabel="Keep editing"
              confirmLabel="Discard and exit"
              description="Your progress will be lost. You can start a new planCreation at any time."
              onConfirm={fw.close}
              open={showCancelDialog}
              onOpenChange={onCancelDialogChange}
              title="Exit the planCreation?"
              tone="danger"
              trigger={
                <Button
                  variant="destructive"
      ...
      <PlanBuilderProgressBar
        step={fw.step}
        isPrePlanCreation={fw.isPrePlanCreation}
        groupFormationResult={fw.groupFormationResult}
