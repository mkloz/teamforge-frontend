...
import { ICON_MAP } from "@/features/plan-creation/components/steps/step1-activity/activity-icon-map";
import {
  buildCategoryFitHighlights,
  buildCrossCategoryTemplateSuggestions,
  type SuggestedTemplate,
} from "@/features/plan-creation/lib/plan-template-suggestions";
...
interface PlanCreationTemplateRecommendationsProps {
  onSelect: (templateId: string) => void;
}

export function PlanCreationTemplateRecommendations({
  onSelect,
}: PlanCreationTemplateRecommendationsProps) {
  const currentUser = useQuery(currentUserQueryOptions());

  if (currentUser.isPending) {
    return <PlanCreationTemplateRecommendationsSkeleton />;
  }
  ...
  return (
    <section aria-labelledby="plan-creation-recommendations-title">
      <div>
        <h2
          id="plan-creation-recommendations-title"
        ...
        <p className="mt-2 max-w-2xl text-muted-foreground text-sm leading-relaxed">
          {isPersonalized
            ...
            : "Ready-to-use templates from across Findafew."}
        </p>
      </div>
      ...
      {recommendations.map((recommendation, index) => {
        ...
        return (
          <EditorialBentoItem key={recommendation.id} slot={slot}>
            <PlanCreationTemplateRecommendationTile
...
function PlanCreationTemplateRecommendationTile({
  layout,
  onSelect,
  recommendation,
}: {
...
function PlanCreationTemplateRecommendationsSkeleton() {
  return (
    <section
      aria-busy="true"
      aria-labelledby="plan-creation-recommendations-title"
    >
