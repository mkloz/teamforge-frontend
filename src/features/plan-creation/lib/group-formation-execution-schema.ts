...
import { parsePositiveCostAmount } from "@/features/plan-creation/lib/group-formation-activity-builders/cost-amount-parser";
import {
...
const groupFormationExecutionBaseSchema = z.object({
  groupFormationScope: z.enum(["LOCAL", "ONLINE"]),
...
type GroupFormationExecutionBaseInput = z.infer<
  typeof groupFormationExecutionBaseSchema
>;
type GroupFormationExecutionRefinementContext = z.RefinementCtx;

export const groupFormationExecutionInputSchema =
  groupFormationExecutionBaseSchema.superRefine(
    addGroupFormationExecutionIssues,
  );

function addGroupFormationExecutionIssues(
  input: GroupFormationExecutionBaseInput,
  ctx: GroupFormationExecutionRefinementContext,
) {
...
function requiresPlanLocation(input: GroupFormationExecutionBaseInput) {
...
export type AutomaticGroupFormationExecutionInput = z.infer<
  typeof groupFormationExecutionInputSchema
>;
