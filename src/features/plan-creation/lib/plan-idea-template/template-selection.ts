...
import { resolveCanonicalActivityTemplate } from "@/features/plan-creation/public/canonical-activity-templates";
...
export function selectPlanIdeaTemplate(
  idea: PlanIdeaLaunch,
): PlanIdeaTemplateSelection | null {
...
function selectTemplateSeed(idea: PlanIdeaLaunch): ScoredTemplateSeed {
  return (
...
function getPreferredSeedId(idea: PlanIdeaLaunch) {
...
function findLaneDefaultTemplateSeed(idea: PlanIdeaLaunch) {
...
function getLanePairKey(idea: PlanIdeaLaunch) {
...
function findHighestScoringTemplateSeed(idea: PlanIdeaLaunch) {
...
function getCatalogFallbackTemplateSeed() {
  ...
  if (!fallback) {
    throw new Error(
      "The planCreation template catalog has no fallback template.",
    );
  }
